...
class SampleConvNet(nn.Module):                                                 
    def __init__(self):                                                         
        super().__init__()                                                      
        self.conv1 = nn.Conv2d(1, 16, 3, padding=1) # <-- change kernel size and remove stride                         
        self.fc1 = nn.Linear(16 * 28 * 28, 10) # <-- change input and output dimensions
                                                                                
    def forward(self, x):                                                       
        # x of shape [B, 1, 28, 28]                                             
        x = F.relu(self.conv1(x))  # -> [B, 16, 28, 28]  # <-- change output shape
        x = x.view(-1, 16 * 28 * 28)  # -> [B, 12544] # <-- change input dimension                              
        x = self.fc1(x)  # -> [B, 10]                                           
        return x 
